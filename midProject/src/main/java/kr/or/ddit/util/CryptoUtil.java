...
public class CryptoUtil {
	...
	public static String encryptAES256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		...
		// CBC : Cipher Block Chaining Mode ==> 이전 단계의 암호환 결과가
		//		 현 단계의 함호화에 사용되는 방식을 말한다.
		//		 블록 암호화 운영 모드 중 보	안성이 가장 높은 암호화 방법으로
		//		 가장 많이 사용되는 방식이다.
		//		 이것은 암호화가 병렬적으로 처리되는 것이 아니라, 순차적으로 수행되어야
		//   	 한다는 단점이 있다.
		
		// 패딩 : Padding ==> 마지막 블록이 블록의 길이와 항상 딱 떨어지지
		//			않기 때문에, 부족한 길이만큼 '0'으로 채우거나 임의의 비트값들로
		//			채워 넣는 방식을 말한다.
	...
	public static String decryptAES256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, GeneralSecurityException {
		...
		// 암호를 옵션 종류에 맞게 초기화 한다.
		// 옵션 종류 : ENCRIPY_MODE(암호화 모드), DECRIPY_MODE(복호화 모드)
